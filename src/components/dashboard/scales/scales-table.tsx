...
const columns = [
			...
			<Stack direction="row" spacing={1} sx={{ alignItems: "center" }}>
				<div>
					<Link
						...
						href={paths.dashboard.scales.details(row.id)}
	...
	{
		formatter: (row): React.JSX.Element => (
			<IconButton component={RouterLink} href={paths.dashboard.scales.details(row.id)}>
				<PencilSimpleIcon />
			</IconButton>
		),
		name: "Actions",
...
] satisfies ColumnDef<Scale>[];

export interface ScalesTableProps {
	rows: Scale[];
}

export function ScalesTable({ rows }: ScalesTableProps): React.JSX.Element {
	const { deselectAll, deselectOne, selectAll, selectOne, selected } = useScalesSelection();
